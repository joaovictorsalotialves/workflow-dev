  ...
  it('Não deve retornar uma editora com id inválido', done => {
    ...
    chai
      ...
      .end((_err, res) => {
        ...
        expect(res.body).to.have.property('message').eql(`id ${idEditora} não encontrado`)
        done()
      })
  })
})
...
describe('DELETE em /editoras', () => {
  ...
  it('Não deve deletar uma editora com id inválido', done => {
    ...
    chai
      ...
      .end((_err, res) => {
        expect(res.status).to.equal(404)
        expect(res.body).to.have.property('message').eql(`Editora com id ${idEditora} não encontrada`)
        done()
      })
  })
})
